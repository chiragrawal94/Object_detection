...
class PR:
    ...
    def average_prec(self, n_samples=11):
        recall_at = []
        max_prec = 0
        for p, r in self.curve[::-1]:
            while len(recall_at) < (n_samples-1)*(1-r):
                recall_at.append(max_prec)
            max_prec = max(max_prec, p)
        while len(recall_at) < n_samples:
            recall_at.append(max_prec)
        return sum(recall_at) / len(recall_at)
...
class DetectionGrader(Grader):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for img, *gts in self.module.utils.DetectionSuperTuxDataset('dense_data/valid', min_size=0):
            d = det.detect(img.to(device))
            for i, gt in enumerate(gts):
                self.pr_box[i].add([(float(j[1]), float(j[2]), float(j[3])) for j in d if j[0] == i], gt)
                self.pr_dist[i].add([(float(j[1]), float(j[2]), float(j[3])) for j in d if j[0] == i], gt)
...
class DetectionSizeGrader(Grader):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for img, *gts in self.module.utils.DetectionSuperTuxDataset('dense_data/valid', min_size=0):
            d = det.detect_with_size(img.to(device))
            for i, gt in enumerate(gts):
                self.pr[i].add([(float(j[1]), float(j[2]), float(j[3]), float(j[4]), float(j[5])) for j in d if j[0] == i], gt)
